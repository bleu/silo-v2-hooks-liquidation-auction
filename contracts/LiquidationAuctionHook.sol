 ...
 * runs for a fixed number of blocks (~20 minutes), and the highest bidder from the previous
 * auction becomes the authorized liquidator for the current period.
 *
 * Key features:
 *  - Auction periods based on block numbers (100 blocks ≈ 20 minutes)
    ...
    PartialLiquidation
{
    ...
    uint256 public constant AUCTION_BLOCKS = 100; // Blocks per auction (approx. 20 minutes)
