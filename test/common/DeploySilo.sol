...
contract DeploySilo {
    function deploySilo(
        ...
        ISiloDeployer.ClonableHookReceiver
            memory _clonableHookReceiver = ISiloDeployer.ClonableHookReceiver({
        ...
        returns (ISiloDeployer.Oracles memory oracles)
    {
        ...
        oracles.solvencyOracle0.factory = address(
            new ChainlinkV3OracleFactory()
        );
        ...
        config.primaryAggregator = AggregatorV3Interface(
            ArbitrumLib.CHAINLINK_ETH_USD_AGREGATOR
        );
        ...
        oracles.solvencyOracle0.txInput = abi.encodeWithSelector(
            ChainlinkV3OracleFactory.create.selector,
            config
        );
    }

    function _irmConfigData()
        internal
        pure
        returns (IInterestRateModelV2.Config memory irmConfigData)
    {
    ...
    function _siloInitData()
        internal
        view
        returns (ISiloConfig.InitData memory siloInitData)
    {
